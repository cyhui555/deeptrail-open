...
import { inspectIssueWorkItemProposal } from "./work-item-proposal.mjs";
...
async function main() {
  ...
  if (args.length !== 2 || args[0] !== "--issue" || !/^[1-9]\d*$/.test(args[1])) {
    throw new LoopGatewayError(
      "USAGE",
      "用法：loop:work-item-proposal -- --issue <positive-number>"
    );
  }
  const result = await inspectIssueWorkItemProposal(Number(args[1]));
