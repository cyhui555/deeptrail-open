...
- 公开主仓已登记且绑定 Evidence 8/10、共 15 个适用 Profile Run；首次成功率、复用率与闭环率均为 100%，边界违规 0、连续成功 8。第 5–8 项均在登记进入受保护 `main` 后才运行并绑定证据。
...
修复 `BUG-20260717-006` 的 Release 身份与发布失败自动恢复，进入受保护主干后完成第 8 项 Shadow，再重新发布并复验两个既有 Bug。
