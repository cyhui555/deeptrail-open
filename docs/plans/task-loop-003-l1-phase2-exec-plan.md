...
- [ ] 完成 M1 持续门禁：CI 与公开准备安全门已实现；工程所有者选择 GitHub Free 公开仓库，等待干净公开基线、独立 Reviewer、主干保护和临时直推例外关闭。
- [x] 完成 M2 可复现 Profile。
...
- GitHub 只读 API 证明当前仓库为 private，Branch Protection 与 Rulesets 因账户方案限制返回 403；公开前审计未发现高置信源码/日志密钥，但旧历史含目标地址，且一个历史 E2E 产物含仍有效的本地测试 Token，因此不能直接切换现仓可见性。

### 已做决策

- 下一阶段目标为 L1 Phase 2，不直接进入 L2/L3。
- 工程所有者已选择零成本的 GitHub Free 公开仓库；采用脱敏当前快照建立全新公开主仓，原私有仓库保留为只读审计档案，避免暴露旧 PR、Actions 与制品历史。
- L2 保持 Proposal-only，任何 Mutation 需要新的 Requirement、ADR 和专项验收。
- 先恢复持续可信度，再扩展 Profile；先建立业务基线，再讨论自治。
- M4 不因闭环和边界指标达标而自动批准 L2；首次成功率为 0、主干保护未恢复和目标环境未复验期间保持 L1，L2 仅保留不可写 Proposal 能力。

## 11. 下一项唯一动作

由工程所有者确认公开仓库名并提供至少一名独立 Reviewer；随后用 `pnpm security:public-prepare` 生成单根脱敏基线，首次远端 CI 全绿后应用 `.github/branch-protection-main.json`，以真实 PR 验证并关闭 `TASK-GOV-001`，再创建目标环境发布验证。
