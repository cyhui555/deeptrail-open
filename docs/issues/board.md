...
- `TASK-OPS-002`：零付费、无生产密钥且测试阶段断网的目标机回归已固化，见[活动 Work Item](task-ops-002-no-cost-target-regressions.md)。
...
- `TASK-LOOP-003`：PR #22–#29、目标机发布和自动恢复均已实证；Cohort 已登记 10/10，待最后两项 Shadow 绑定后执行严格准入，见[活动 Work Item](task-loop-003-l1-phase2-to-l2.md)、[执行计划](../plans/task-loop-003-l1-phase2-exec-plan.md)与[准入报告](../verification/task-loop-003-m4-l2-admission.md)。
