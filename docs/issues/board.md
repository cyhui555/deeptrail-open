...
- `TASK-GIT-001`：建立默认只读、显式批准的 Post-merge GC，收口短期分支与 linked worktree，见[活动 Work Item](task-git-001-post-merge-gc.md)与[执行计划](../plans/task-git-001-post-merge-gc-exec-plan.md)。
...
- `TASK-LOOP-007`：只读 Issue Intake 已由 PR #46 合入，见[交付摘要](../archive/task-loop-007-autonomous-intake.md)。
