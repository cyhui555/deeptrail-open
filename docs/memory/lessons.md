...
## LESSON-015：维护自动化必须服从产品主线和硬预算

- 状态：已确认
- 证据：`main@b46d660..714a633` 的 30 个提交中，按主题粗分仅 6 个属于产品/缺陷，13 个属于 Loop、11 个属于治理/文档/依赖/发布；5 条 Loopany 循环累计 40 次运行，React Doctor 5 次与 Housekeeper 7 次均未产生已落地主线价值；PR #57 引入的高权限归档控制面又由 PR #59 撤回。App 任务还曾把“地理位置失败”误解为手机 GPS，合并后又启动重叠 G3 文档同步；PR #67 正文首次写入发生乱码，均在复核后停止或修正。
- 根因：把自动化等级、健康分、治理闭环和运行次数当成产品进展，且开放循环缺少总 WIP、连续无结果暂停线和与产品任务的互斥门禁；同时在用户语义尚未映射到真实业务链路前开始实现、共享状态没有单写者、外部写入缺少读回校验，都会生成返工、重叠事实和状态漂移。
- 适用范围：会扫描代码、修改源码、创建分支/PR 或反复生成工程报告的定时维护与自治试验。
- 不适用范围：明确 P0/P1 的生产或安全事件；此类中断仍必须有严重级别、负责人、退出条件和恢复主线动作。
- 结论：产品 WIP 固定为 1；代码型维护循环默认不与产品任务并行。新循环必须声明产品关联、单轮产出、日运行/开放 PR 上限和暂停条件；连续三轮无可执行结果、写路径长期阻塞或与活动工作重叠时暂停。歧义需求先绑定用户可见链路与验收再改码；共享看板/状态同一时段只允许一个写者；PR、配置等外部写入必须读回正文、精确 Head 与生效状态。
- 验证：React Doctor 与 Housekeeper 已读回 `enabled=false`、`nextFire=null`；PR #67 不含误向 GPS 改动，正文已读回且中文可读，重复 G3 worktree 已回收；活动看板只保留 Android 主线及其子 Bug，历史 Loop/治理文档退出默认导航，活动文档预算收紧为 2600 行。
- 晋升位置：`AGENTS.md`、执行看板、文档治理与 Loopany 配置
- 共享晋升：未申请；中央校验 8/8 通过，但受保护脏现场已有 Deeptrail 记忆重叠改动，本轮未写入中央
