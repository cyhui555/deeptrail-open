...
- 最后核对：2026-07-19
- 当前阶段：`TASK-RELEASE-004` G3 + `TASK-LOOP-008` G2
- 当前检查门：公开 `main@88b5092`；远程制品 Workflow 已合入，真实制品运行尚未执行
...
由所有者配置 `release-artifacts` Environment 的两项 Web 构建值，再对当前受保护 `main` 手工执行一次远程制品运行并核验，仍不部署。
