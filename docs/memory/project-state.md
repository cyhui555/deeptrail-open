...
- 当前检查门：PR #22 已完成单维护者受保护合并实证；公开 Cohort 当前 3/10，第 4 项 `TASK-CI-001` 已预登记、待受保护合并后运行 Shadow，两个 Bug 仍待不可变发布与目标环境复验
- 活动工作项：`BUG-20260717-001`、`BUG-20260717-002`、`TASK-CI-001`
...
- 工程所有者已授权：提交 GitHub Issue 或 Bug 时，同步登记本地 Work Item/看板并执行适用检查；`TASK-GOV-001` 关闭后禁止直推 `main`，所有变更经机器人机制作者 PR、唯一人工所有者审批和五项 Required Checks 合并，不自动部署。产品继续保留现有路由和框架主版本，不引入多人协作、票务接入或基础设施迁移。
...
- LoopAny：四类最终 Profile 均首次通过且重复零命令复用；机器人作者 PR #22 经所有者批准和五项必需检查后合并为公开 `main@d70963c`，保护规则已由 API 读回且无漂移。
- 历史本地全量为 118 通过、1 条件跳过；GitHub 手工全量 run `29577848547` 为 107 通过、1 跳过、21 失败，已定位为 AMap 测试 key、PDF 静态地图隔离和 development Service Worker 用例误跑 production 三类真实问题，后续必须分别登记修复。
...
合并已预登记的 `TASK-CI-001` 并运行第 4 项 Shadow，再进入两个 Bug 的不可变发布复验和其余 6 项预登记真实样本。
