...
- 当前检查门：等待唯一人工所有者批准 `github-actions[bot]` 创建、锁定 SHA 的治理 PR；该账号级分离不冒充第二位人员审计。公开新样本已完成 3/10，`BUG-20260717-001/002` 仍待不可变发布与目标环境复验
...
- LoopAny：四类最终 Profile 均首次通过且重复零命令复用；公开 `main@daab55f` 的引导 PR 五项必需检查全绿，保护规则恢复后已由 API 读回，机器人作者治理 PR 尚待所有者批准。
...
由唯一人工所有者批准已整合全部证据的机器人作者治理 PR并关闭 `TASK-GOV-001`，再进入两个 Bug 的不可变发布复验和其余 7 项预登记真实样本。
