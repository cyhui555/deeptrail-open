...
- 当前检查门：公开 `main@020638c` 与远端一致、CI 成功；公开 Home Doctor 与严格 Cohort 48/48 v2、10 Work Items / 17 Profiles 通过；当前分支 Loop 单测 23/23
...
- 旧手册曾把公开仓指向历史私有 Home；该 Home 的 34 份 v2 + 五份 v1 已由固定 Backup 逐字节证明，正式公开 `deeptrail-open-loop` 未发生 Cohort 回退。
...
- 公开 Home：Doctor 无 Writer/未终结事务；`pnpm loop:cohort:l2:strict` 以 48/48 v2、10/10 Work Items、17/17 Profiles 通过。
- 历史 Home：`loop:receipts:verify` 为 34 份 v2 + 五份固定 Backup 证明 v1，`unattestedLegacy=0`。
