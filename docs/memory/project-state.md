...
- 当前检查门：公开 `main@a79f9af`；`bug/e2e-production-remediation` 只允许形成 Draft PR，等待人工 Review/Merge
...
## 下一项唯一动作
推送 `bug/e2e-production-remediation` 并创建 Draft PR 交人工审核；合入后另行批准配置 Key、不可变部署与生产全量 E2E 复验。
