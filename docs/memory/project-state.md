...
- 最后核对：2026-07-20
- 当前阶段：`TASK-APP-001` 真机反馈子项 `BUG-20260720-002` G2；实现与本地门禁通过，等待审查、合入及真机复验
- 公开事实源：`main@714a633`，本地 `main` 与 `origin/main` 一致
- 活动工作项：`TASK-APP-001`；`BUG-20260720-001/002` 是其子项，`TASK-RELEASE-004` 仅为运维跟进
...
- `BUG-20260720-002` 修复前失败证据已确认；修复后地图路线、标记生命周期与刷新初态 15/15，Server 681/681，以及 lint、typecheck、build、文档和 Work Item 门禁通过；未调用真实高德服务。
...
审查并合入 `BUG-20260720-002`，再将更新后的受检主干发布到受控验收环境，执行现场切天、坐标补全率与 360px/390px 既定真机复验；不同时开启其他功能。
