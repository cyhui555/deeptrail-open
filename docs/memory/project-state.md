...
- 当前检查门：等待真实独立 Reviewer 完成已整合治理证据与 Cohort 门禁的 PR #17；公开新样本已完成 3/10，`BUG-20260717-001/002` 仍待不可变发布与目标环境复验
...
由真实独立 Reviewer 审核已整合全部证据的 PR #17并关闭 `TASK-GOV-001`，再进入两个 Bug 的不可变发布复验和其余 7 项预登记真实样本。
