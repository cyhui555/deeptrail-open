...
- 当前能力：正式公开 L2 Proposal-only；L3A 已激活并通过真实试点；L3B Engine 已实现但默认关闭
- 不包含：业务写入、Daemon/Cron、自动 Skill Apply、自动审批/部署、未独立 activation 的 L3B 和 L3C
...
### 10.1 L3B 受控合并 Engine

Engine 提供以下命令，但仓库内 `l3b-policy.json` 当前为 `l3b-disabled`，因此两者都必须失败关闭；只有独立 activation PR 绑定 Engine 批准 Head、合入 Revision、Review URL、稳定 Cohort 与当前保护摘要后才可使用：
