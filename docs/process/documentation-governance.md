...
5. `project-state.md` 只保留阶段、活动项、阻塞、最后验证和一个下一动作，最多 60 行。
6. 单个 Markdown 最多 320 行；手工 API 入口最多 160 行；不含 `docs/archive/` 的活动 Markdown 总量最多 3400 行。历史归档仍受单文件预算约束，但不挤占活动文档预算。
7. 原始截图、PDF、日志、测试报告、数据库和生成产物不进入 `docs/` 或工程记忆。
