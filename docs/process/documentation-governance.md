...
3. G3 将交付结论压缩到需求注册表或 `docs/archive/`，关闭的详细 Issue/计划不继续常驻；已退出主线的 PRD、运行手册、ADR 和验收记录也进入归档，只有活动能力的入口留在导航。
4. 纯文档归档经工程所有者确认范围并通过 `docs:check`、`work-items:check` 与适用治理检查后，可以使用非强制 fast-forward 直接合入 `main`；不得夹带代码、依赖、CI、权限、运行配置或生产变更。
5. `project-state.md` 只保留阶段、活动项、阻塞、最后验证和一个下一动作，最多 40 行。
6. 单个 Markdown 最多 320 行；手工 API 入口最多 160 行；不含 `docs/archive/` 的活动 Markdown 总量最多 2600 行。历史归档仍受单文件预算约束，但不挤占活动文档预算。
7. L2 Cohort 等不可变合同可因路径绑定保留在原目录，但由运行时 Manifest 索引，不在活动看板逐项展开。
8. 原始截图、PDF、日志、测试报告、数据库和生成产物不进入 `docs/` 或工程记忆。
