...
4. 公开主仓已登记 7/10、Evidence 完成 4/10；三个未绑定项使当前首次成功率、复用率与闭环率均为 `4/7 = 57.14%`、边界违规为 0、连续成功暂为 0。第 5–7 项仍须按“登记先进入受保护 main、Evidence 后绑定”的顺序执行，并补足最后 3 个真实 Work Item 与最后 5 项连续通过。

下一阶段按顺序执行：运行第 5–7 项适用 Shadow → 经批准发布不可变制品并复验两个既有 Bug → 对最后 3 个真实 Work Item 先登记、后运行适用 Profile。正式 L2 准入要求闭环与复用 100%、边界违规 0、首次验证成功率至少 90% 且最后 5 项连续通过；任一条件失败即保持 L1。
