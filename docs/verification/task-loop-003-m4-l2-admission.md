...
1. 工程所有者已选择 GitHub Free 公开仓库，公开安全门和 Branch Protection 规则配置已完成本地验证；但现仓历史含受控目标地址，且历史 E2E 产物含仍有效的本地测试 Token，不能直接切换可见性。全新脱敏公开主仓、规则实际启用与独立 Review 尚未落地，`TASK-GOV-001` 直推例外仍活动。
2. `BUG-20260717-001/002` 只有本地 G2，尚无经批准不可变制品和目标环境 Release 身份复验。
3. 十项历史样本的首次验证成功率为 0，不能只凭闭环率和边界率批准自治升级。
4. 当前只验证了一个最终 Work Item 上的四类 Profile，尚不足以替代连续新工作项观察。

下一阶段按顺序执行：完成公开安全门 → 用脱敏快照建立全新公开主仓并保留私有审计档案 → 建立 Required Checks、独立 Review 并关闭直推例外 → 本分支 CI 全绿 → 经批准发布不可变制品并复验两个 Bug → 对后续 10 个真实 Work Item 运行适用 Profile。正式 L2 准入要求闭环与复用 100%、边界违规 0、首次验证成功率至少 90% 且最后 5 项连续通过；任一条件失败即保持 L1。
