...
下一阶段按顺序执行：合并 `TASK-CI-001` 预登记并运行第 4 项 Shadow → 经批准发布不可变制品并复验两个 Bug → 对其余 6 个真实 Work Item 先登记、后运行适用 Profile。正式 L2 准入要求闭环与复用 100%、边界违规 0、首次验证成功率至少 90% 且最后 5 项连续通过；任一条件失败即保持 L1。
