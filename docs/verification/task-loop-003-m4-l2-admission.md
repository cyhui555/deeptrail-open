...
4. 公开主仓新样本当前为 4/10、7 个适用 Profile Run；首次成功率、复用率与闭环率均为 100%，边界违规为 0、连续成功 4，仍缺后续 6 个真实 Work Item 与最后 5 项连续通过。

下一阶段按顺序执行：合入三个真实 Full E2E 失败簇并预登记第 5–7 项 → 经批准发布不可变制品并复验两个既有 Bug → 对其余 3 个真实 Work Item 先登记、后运行适用 Profile。正式 L2 准入要求闭环与复用 100%、边界违规 0、首次验证成功率至少 90% 且最后 5 项连续通过；任一条件失败即保持 L1。
