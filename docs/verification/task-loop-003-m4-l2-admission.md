...
4. 公开主仓已登记且绑定 Evidence 7/10、共 13 个适用 Profile Run；首次成功率、复用率与闭环率均为 100%，边界违规为 0、连续成功 7，尚缺最后 3 个真实 Work Item。

下一阶段按顺序执行：经批准发布不可变制品并复验两个既有 Bug → 对最后 3 个真实 Work Item 先登记、后运行适用 Profile。正式 L2 准入要求闭环与复用 100%、边界违规 0、首次验证成功率至少 90% 且最后 5 项连续通过；任一条件失败即保持 L1。
