...
async function captureVisualEvidence(page: Page, fileName: string): Promise<void> {
  ...
  // 截图只使用本文件的固定夹具，并在可见行为断言通过后生成，避免上传失败现场或真实用户数据。
...
test('月历逐日展示行程范围，删除需确认且失败时保留原行程', async ({ page }) => {
  ...
  await captureVisualEvidence(page, 'trip-calendar-desktop.png');

  await calendar.getByRole('button', { name: '删除行程：待定周末' }).click();
  const confirmDialog = page.getByRole('alertdialog', { name: '删除这个行程？' });
  await expect(confirmDialog).toBeVisible();
  await captureVisualEvidence(page, 'trip-delete-confirm-desktop.png');
...
test('390px 与 360px 使用迷你月历和当日议程且没有横向溢出', async ({ page }) => {
  ...
  await captureVisualEvidence(page, 'trip-calendar-mobile-390.png');
...
test('最近任务按类型与状态组合筛选，翻页沿用相同条件', async ({ page }) => {
  ...
  await captureVisualEvidence(page, 'recent-task-filters-desktop.png');
